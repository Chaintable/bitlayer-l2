...
func BuildPipelineTransaction(tx *types.Transaction, receipt *types.Receipt, from common.Address, baseFee *big.Int) ptypes.Transaction {
	...
	nonce := tx.Nonce()
	// Note: Removed Blast-specific DepositNonce handling for bitlayer-l2
	// Blast/Optimism has DepositNonce for L1->L2 deposit transactions
	// bitlayer-l2 uses standard Ethereum transaction structure

...
type callTracer struct {
	...
	gasLimit  uint64
	depth     int
	interrupt atomic.Bool // Atomic flag to signal execution interruption, authorizationResults []types.AuthorizationResult
...
func newCallTracerRaw(ChangeContracts map[common.Address]struct{}, BlockFile *ptypes.BlockFile) *callTracer {
	t := &callTracer{callstack: make([]callFrame, 0, 1), config: callTracerConfig{
		...
		BlockFile: BlockFile, ChangeContracts: ChangeContracts,
	}
	return t
}
...
func (t *callTracer) OnTxEnd(receipt *types.Receipt, err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			log.Error("Recovered in OnTxEnd", "receipt", receipt)
...
func setStorageChange(cf *callFrame, ChangeContracts map[common.Address]struct{}) {
	if cf.To != nil && cf.SelfStorageChange {
		// Note: bitlayer-l2 doesn't support EXTDELEGATECALL (EIP-7702) yet
		// Only check for standard DELEGATECALL
